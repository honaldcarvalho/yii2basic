...
class InputImage extends Widget
{
    ...
    public function run()
    {
      ...
      JS;
        ...
        HTML;
        if($this->as_type == 1){
            $field = '<textarea class="d-none" type="text" id="'.$this->field_id.'" name="'.$this->field_name.'">'.$this->model.'</textarea>';
        }else{
            $field = '<input type="text" id="'.$this->field_id.'" name="'.$this->field_name.'" value="'.$this->model.'" />';
        }
        $form_upload = <<< HTML
            {$modal}
